...
public:
	...
	linkadv(void)
	{
		...
		int rc = ioctl(sock, SIOCETHTOOL, &ifr);
		if (rc < 0) {
			perror("ethtool ioctl");
			return -1;
		}
		return ethcmd.advertising;
	}
...
main(int argc, char **argv)
{
	...
	if (linkstatus) {
		linkadv = peth->linkadv();
		linklpadv = peth->linklpadv();
		linksupported = peth->linksupported();
	}
	printf("%x %x %x\n", linkadv, linklpadv, linksupported);
