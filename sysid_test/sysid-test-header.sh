...
function apply_overlay()
{
    ...
    echo "echo \"$image\" > /sys/class/fpga_region/${region}/firmware_name"
    echo "$image" > /sys/class/fpga_region/${region}/firmware_name
    
    exit_if_fail $? "echo \"$image\" > /sys/class/fpga_region/${region}/firmware_name"
}
...
function remove_overlay()
{
    ...
    echo "echo > /sys/class/fpga_region/${region}/firmware_name"
    echo > /sys/class/fpga_region/${region}/firmware_name
    
    exit_if_fail $? "echo > /sys/class/fpga_region/${region}/firmware_name"
}
...
Test the FDT header on FPGA images
...
STATICREGION=ghrd-fit0.fit
PERSONA0=ghrd-fit1.fit
PERSONA1=ghrd-fit2.fit
...
if [ -n "$STATICREGION" ]; then
    ...
    apply_overlay ${STATICREGION} region0
fi
...
apply_overlay ${PERSONA0} region1
sysfs_cat_test $FPGA_MGR_SYSFS/state 'operating'
echo
ls /sys/class/fpga_region/ -l
check-sysid ff200000 3221756416
...
apply_overlay ${PERSONA1} region1
sysfs_cat_test $FPGA_MGR_SYSFS/state 'operating'
ls /sys/class/fpga_region -l
check-sysid ff200000 3221756416
check-sysid ff200800
